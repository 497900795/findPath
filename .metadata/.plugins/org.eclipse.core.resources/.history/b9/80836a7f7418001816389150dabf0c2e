			...
			if(find){
				...
				for(int i = 0;!can;i++){//楼层
					for(int j = 0;j < NUM;j++){//点
						if(transBulidingWay[startBuliding][startFloor-j][i] == sureBulidingWay()){
							can = true;
							startFloor = startFloor - j;
							break;
						}
						else if(transBulidingWay[startBuliding][startFloor+j][i] == sureBulidingWay()){
							can = true;
							startFloor = startFloor + j;
							break;
						}
